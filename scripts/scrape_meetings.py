#!/usr/bin/env python3
"""
Scrape and analyze board-meeting agenda/minutes documents for turf mentions,
from any supported source platform (see scripts/platforms/).

This is the platform-neutral successor of scrape_boardbook.py (which remains
as a thin back-compat wrapper). Fetching - listing an org's meetings and
downloading its documents - is delegated to a per-platform adapter; everything
else here (text extraction, turf-term matching, classification, the hybrid
minutes pass, scrape state, output files) is shared and identical across
platforms.

Usage:
    python3 scrape_meetings.py --org 795 --limit 10                  # BoardBook (default)
    python3 scrape_meetings.py --platform sparq --org 120 --limit 5  # Omaha on Sparq
    python3 scrape_meetings.py --platform boeconnect --org kcs --limit 5
    python3 scrape_meetings.py --platform boarddocs --org ny/albany --limit 5

Org ids are strings: numeric ("795") or slug ("kcs", "papillion-lavista") for
the BoardBook family, "{state}/{slug}" ("ny/albany") for BoardDocs.

By default, downloaded documents are analyzed in-memory and then discarded
(see docs/DATA_STORAGE.md). Pass --keep-pdfs to retain them on disk.
"""
import argparse
import csv
import io
...
import scrape_state
from platforms import DEFAULT_PLATFORM, MeetingRef, get_adapter, implemented_platforms
...
class AnalysisResult:
    ...
    minutes_available: bool = False          # a minutes document was posted and fetched
    ...
    minutes_pages: int = 0
    # Platform provenance (repo-internal output fields, not part of the lead
    # contract): which platform/org this document came from, and the
    # human-facing deep links the export step reuses as source URLs.
    platform: str = ""
    platform_org_id: str = ""
    org_url: str = ""
    agenda_url: str = ""
...
def extract_text(document_bytes: bytes) -> tuple:
    """Return (full_text, page_count) from raw document bytes.

    Documents are PDFs on the BoardBook family but HTML on BoardDocs, so the
    format is sniffed rather than assumed: %PDF magic -> PyPDF2 per-page
    extraction (memory-bounded, page numbers citable); anything with markup ->
    one HTML text pass (page_count 1, since HTML has no pages). This keeps the
    analysis pipeline platform-agnostic - adapters return bytes, nothing more.
    """
    if looks_like_pdf(document_bytes):
    ...
    head = document_bytes[:2048].lstrip().lower()
    if head.startswith(b"<") or b"<html" in head or b"<div" in head:
        soup = BeautifulSoup(document_bytes, "lxml")
        text = soup.get_text(" ", strip=True)
        return (f"[Page 1] {text}" if text else ""), (1 if text else 0)

    # Unknown format: let PyPDF2 raise so the caller records a parse_error,
    # the same way a corrupt PDF is handled.
    reader = PdfReader(io.BytesIO(document_bytes))
...
def _document_filename(meeting: MeetingRef, document_bytes: bytes, suffix: str = "") -> str:
    safe_title = re.sub(r"[^A-Za-z0-9_\-]+", "_", meeting.date_str)[:60]
    ext = "pdf" if looks_like_pdf(document_bytes) else "html"
    return f"{meeting.meeting_id}_{safe_title}{suffix}.{ext}"
...
def process_meeting(
    ...
    document = adapter.fetch_document(org_ref, meeting.meeting_id, "agenda")
    if document is None:
        result = AnalysisResult(meeting=meeting, turf_mentioned=False, error="download_failed")
    else:
        try:
            text, pages = extract_text(document)
        except Exception as e:  # malformed/encrypted PDF, etc.
            result = AnalysisResult(meeting=meeting, turf_mentioned=False, error=f"parse_error: {e}")
        else:
            result = analyze_text(meeting, text, pages, len(document))

            if keep_pdfs:
                pdf_dir.mkdir(parents=True, exist_ok=True)
                (pdf_dir / _document_filename(meeting, document)).write_bytes(document)
            # else: document bytes go out of scope here and are garbage-
            # collected; nothing touches disk. See docs/DATA_STORAGE.md.

            # Hybrid minutes pass: only for turf hits, and only when minutes
            # exist. The agenda is scraped for every meeting (discovery); the
            # minutes are fetched just for the handful of hits to confirm what
            # was decided.
            if result.turf_mentioned and not skip_minutes:
                minutes_bytes = adapter.fetch_document(org_ref, meeting.meeting_id, "minutes")
                if minutes_bytes is not None:
                    result.minutes_available = True
                    result.minutes_url = adapter.document_page_url(org_ref, meeting.meeting_id, "minutes")
                    try:
                        ...
                        # Minutes existed but couldn't be parsed; leave outcome
                        # unset. minutes_available stays True so this is
                        # distinguishable from "no minutes posted".
                        pass
                    if keep_pdfs:
                        (pdf_dir / _document_filename(meeting, minutes_bytes, "_minutes")).write_bytes(minutes_bytes)

    # Provenance stamps, present on every record (including failed downloads,
    # so a record is always traceable to its platform/org).
    result.platform = adapter.platform
    result.platform_org_id = str(org_ref)
    result.org_url = adapter.org_page_url(org_ref)
    result.agenda_url = adapter.document_page_url(org_ref, meeting.meeting_id, "agenda")
    return result
...
def result_to_record(r: AnalysisResult) -> dict:
    ...
    return {
        ...
        # Platform provenance (repo-internal; the lead contract is unchanged -
        # export_leads.py folds these into evidence.details / source URLs).
        "platform": r.platform,
        "platform_org_id": r.platform_org_id,
        "org_url": r.org_url,
...
def merge_records(prior: list, new: list) -> list:
    ...
    (Output files are per platform-org, so meeting ids cannot collide across
    platforms within one file.)
    """
...
def write_csv(records: list, path: Path):
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["meeting_id", "date", "title", "turf_mentioned", "num_matches", "topic_types", "sentiments", "outcomes", "minutes_outcome", "summary", "pages", "error", "platform", "platform_org_id"]
        )
        for r in records:
            ...
            writer.writerow(
                [
                    ...
                    r.get("error") or "",
                    r.get("platform") or "",
                    r.get("platform_org_id") or "",
                ]
            )


def main():
    parser = argparse.ArgumentParser(description="Scrape and analyze board-meeting documents for turf mentions.")
    parser.add_argument("--platform", default=DEFAULT_PLATFORM,
                        help="Source platform: %s (default: %s)"
                             % (", ".join(implemented_platforms()), DEFAULT_PLATFORM))
    parser.add_argument("--org", required=True,
                        help="Platform-scoped organization id: numeric or slug for the "
                             "BoardBook family (795, kcs), state/slug for BoardDocs (ny/albany)")
    ...
    parser.add_argument("--keep-pdfs", action="store_true", help="Persist downloaded documents to disk instead of discarding them")
    parser.add_argument("--pdf-dir", default="output/pdfs", help="Directory to store documents if --keep-pdfs is set")
    ...
    parser.add_argument("--skip-minutes", action="store_true",
                        help="Do not fetch the minutes to confirm outcomes for turf hits "
                             ...
                             "minutes document, when one exists, to record the decision)")
    ...
    org_ref = args.org.strip()
    # The platform-namespaced key used for all scrape-state bookkeeping:
    # meeting ids are only unique within one platform.
    state_org_key = scrape_state.org_key(adapter.platform, org_ref)
    ...
    if args.meeting_id:
        ...
        print(f"Fetching meeting list for {adapter.platform} org {org_ref}...", file=sys.stderr)
        meetings = adapter.list_meetings(org_ref)
    ...
    if state is not None and not args.meeting_id and not args.force_rescrape:
        ...
        skipped = 0
        for m in meetings:
            entry = scrape_state.get_entry(state, state_org_key, m.meeting_id)
            ...
            if reason in ("document_overdue", "minutes_overdue"):
                scrape_state.mark_final(state, state_org_key, m.meeting_id, reason, now)
            skipped += 1
        if skipped:
            print(
    ...
    for i, meeting in enumerate(meetings, 1):
        ...
        result = process_meeting(adapter, org_ref, meeting, args.keep_pdfs, pdf_dir, args.skip_minutes)
        results.append(result)
        if state is not None:
            scrape_state.record_result(
                state, state_org_key, meeting.meeting_id, meeting.date_str,
    ...
    if state is not None:
        scrape_state.touch_org(state, state_org_key, now)
